...
export async function POST(req: NextRequest) {
    ...
    if (typeof action !== 'string' || !['checkout', 'volunteer', 'add', 'remove', 'set', 'create'].includes(action)) {
        return NextResponse.json({ error: 'action must be one of: checkout, volunteer, add, remove, set, create' }, { status: 400 });
    }
